...
// Validate request body when providing custom credentials
...
async function handleConnect(req: NextRequest, isPost = false) {
  try {
    ...
    // Parse credentials from request body if POST
    ...
    // Store credentials in the state if provided
    const stateData = credentials 
      ? { state, credentials } 
      : { state };
    ...
    const authUrl = GoogleAuth.getAuthorizationUrl(credentials, encodedState);
    ...
    if (storedState !== state) {
      alert('State cookie was not set correctly');
