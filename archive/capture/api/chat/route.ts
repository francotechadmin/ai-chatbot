...
export async function POST(request: Request) {
  try {
    ...
    return createDataStreamResponse({
      ...
      onError: (error) => {
        console.error('Dashboard capture chat stream error:', error);
        return 'Oops, an error occurred!';
      },
    });
  } catch (error) {
    return handleApiError(error, 'Dashboard capture chat');
  }
}
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return handleApiError(error, 'Dashboard capture chat delete');
  }
}
