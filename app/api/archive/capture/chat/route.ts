...
import { generateTitleFromUserMessage } from '@/app/(dashboard)/capture/actions';
...
export async function POST(request: Request) {
  try {
    ...
    return createDataStreamResponse({
      ...
      onError: (error) => {
        console.error('Capture chat stream error:', error);
        return 'Oops, an error occurred!';
      },
    });
  } catch (error) {
    return handleApiError(error, 'Capture chat');
  }
}
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return handleApiError(error, 'Capture chat delete');
  }
}
